...
import re
# Local modules
from mldslls.jsonrpc import path_to_uri, path_from_uri
from mldslls.parse_fortran import fortran_file, fortran_ast, process_file, \
    get_paren_level, expand_name, get_line_context
from mldslls.objects import find_in_scope, find_in_workspace, get_use_tree, \
...
from mldslls.intrinsics import get_intrinsic_keywords, load_intrinsics, \
    set_lowercase_intrinsics
log = logging.getLogger(__name__)
logging.basicConfig(filename='C:\\Users\\RAZER\\Desktop\\mldslls_logs\\mldslls_log_1.log', encoding='utf-8', filemode='w', level=logging.DEBUG)
log.debug("Ran the server")
# Global regexes
# FORTRAN_EXT_REGEX = re.compile(r'^\.F(77|90|95|03|08|OR|PP)?$', re.I)
FORTRAN_EXT_REGEX = re.compile(r'^\.mlc$', re.I)
...
class LangServer:
    ...
    def handle(self, request):
        ...
        handler = {
            ...
            "textDocument/documentHighlight": self.serve_documentHighlight,
            "textDocument/semanticTokens/full": self.serve_semanticTokens,
            "initialized": noop,
    ...
    def serve_initialize(self, request):
        ...
        config_path = os.path.join(self.root_path, ".mldslls")
        ...
        if config_exists:
            try:
                        ...
                        if os.path.isdir(dir_path):
                            ...
                            self.post_messages.append(
                                [2, r'Source directory "{0}" specified in '
                                 r'".mldslls" settings file does not exist'.format(dir_path)]
                            )
                        ...
                        if os.path.isdir(ext_source_dir):
                            ...
                            self.post_messages.append(
                                [2, r'External source directory "{0}" specified in '
                                 r'".mldslls" settings file does not exist'.format(ext_source_dir)]
                            )
                ...
                self.post_messages.append([1, 'Error while parsing ".mldslls" settings file'])
        ...
        if self.notify_init:
            self.post_messages.append([3, "MLDSLLS initialization complete"])
        ...
        return {"capabilities": server_capabilities_temp}
        #     "workspaceSymbolProvider": True,
        #     "streaming": False,
        # }
    ...
    def serve_documentHighlight(self, request):
        with open("C:\\Users\\RAZER\\Desktop\\mldslls_logs\\my_log_1.log",'w') as my_log:
            my_log.write("got to document highlight tokens: ")
            my_log.write(str(request["params"]))
        return [{"range": {
    ...
    def serve_semanticTokens(self, request):

        from mldslls.dslf_language_helpers.semantic_token_full_provider import get_semantic_token_data
        file_path = path_from_uri(request["params"]["textDocument"]["uri"])
        semantic_token_data = get_semantic_token_data(file_path)
        with open("C:\\Users\\RAZER\\Desktop\\mldslls_logs\\my_log_2.log",'w') as my_log:
            my_log.write("got to semantic tokens: ")
            my_log.write(str(request["params"]))
            my_log.write("here are the semantic tokens: ")
            my_log.write(str(semantic_token_data))
        return {"data":semantic_token_data}
